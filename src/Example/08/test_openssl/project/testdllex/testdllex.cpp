...
MessageBoxW_T _MessageBoxW = nullptr;

#pragma optimize("", off) 
int WINAPI DetourMessageBoxW(HWND hWnd, LPCWSTR lpText, LPCWSTR lpCaption, UINT uType) {
    VL_DUAL_MODE_BEGIN;

    printf("DetourMessageBoxW >> \n");
    int result = _MessageBoxW(hWnd, lpText, lpCaption, uType);
...
void hook() {
    ...
    do {
        if (MH_OK != MH_Initialize()) { break; }
        if (MH_OK != MH_CreateHook(&::MessageBoxW, &DetourMessageBoxW, reinterpret_cast<LPVOID*>(&_MessageBoxW))) { break; }
        if (MH_OK != MH_EnableHook(&MessageBoxW)) { break; }
    } while (false);
...
void unhook() {
    ...
    do {
        if (MH_OK != MH_DisableHook(&MessageBoxW)) { break; }
        if (MH_OK != MH_Uninitialize()) { break; }
    } while (false);
